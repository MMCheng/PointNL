...
class SPKNNQuery(Function):
    @staticmethod
    def forward(ctx, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor = None, comp: torch.Tensor = None, new_comp: torch.Tensor = None) -> Tuple[torch.Tensor]:
        """
        SPKNN Indexing
        input: nsample: int32, Number of neighbor
               xyz: (b, n, 3) coordinates of the features
               comp: (b, n, 1) in_comp of the features
               new_xyz: (b, m, 3) centriods
               new_comp: (b, m, 1) centriods in_comp
        ...
        assert new_xyz.is_contiguous()
        assert new_comp.is_contiguous()
        b, m, _ = new_xyz.size()
        n = xyz.size(1)
        idx = torch.cuda.IntTensor(b, m, nsample).zero_()
        dist2 = torch.cuda.FloatTensor(b, m, nsample).zero_()
        pointops_cuda.spknnquery_cuda(b, n, m, nsample, xyz, new_xyz, comp, new_comp, idx, dist2)
        return idx
...
spknnquery = SPKNNQuery.apply



class SP_KNNQuery(nn.Module):
    """
    knn query in super point, it can be think as random
    parameters:
        nsample: int32, Maximum number of features to gather in the ball
    """
    def __init__(self, nsample=32):
        super(SP_KNNQuery, self).__init__()
        self.nsample = nsample

    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor = None, comp: torch.Tensor = None, new_comp: torch.Tensor = None) -> torch.Tensor:
        """
        input: xyz: (b, n, 3) coordinates of the features
               new_xyz: (b, n, 3) centriods
               comp: (b, n, 1) in_comp of xyz
               new_comp: (b, m, 1) in_comp of new_xyz

        output: idx: (b, m, nsample)
        """
        # assert xyz.size() == new_xyz.size()
        # assert comp.size() == new_comp.size()
        
        idx = spknnquery(self.nsample, xyz, new_xyz, comp, new_comp)    # (b, m, nsample)

        return idx

class SP_KNNGroup(nn.Module):
    """
    Groups with SPKNNQuery index
    """
    def __init__(self, use_xyz=True):
        super(SP_KNNGroup, self).__init__()
        self.use_xyz = use_xyz

    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, idx: torch.Tensor = None) -> torch.Tensor:
        """
        input: xyz: (b, n, 3)
               features: (b, c, n)
               idx: (b, n, k)
        output: grouped_features: (b, c+3, m, nsample)
              #  grouped_idxs: (b, m, nsample)
        """
        #assert idx != None
        
        xyz_trans = xyz.transpose(1, 2).contiguous()    # (b, n, 3) -> (b, 3, n)
         
        #print()
        grouped_xyz = grouping(xyz_trans, idx)  # (b, 3, n) with idx:(b, n, k) -> (b, 3, n, k)
        grouped_xyz -= xyz_trans.unsqueeze(-1)  # (b, 3, n) -> (b, 3, n, 1) : (b, 3, n, k)
        
        if features is not None:
            grouped_features = grouping(features, idx)  # (b, c, n) with idx:(b, n, k) -> (b, c, n, k)
            grouped_features -= features.unsqueeze(-1) # (b,c,n,k) - (b, c, n, 1)
            if self.use_xyz:
                new_features = torch.cat([grouped_xyz, grouped_features], dim=1)  # (b, c+3, n, k)
            else:
...
class SP_KNNGroup_V1(nn.Module):
    """
    Groups with SPKNNQuery index
    """
    def __init__(self, use_xyz=True):
        super(SP_KNNGroup_V1, self).__init__()
        self.use_xyz = use_xyz
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, idx: torch.Tensor = None) -> torch.Tensor:
        ...
        if features is not None:
            grouped_features = grouping(features, idx)  # (b, c, n) with idx:(b, n, k) -> (b, c, n, k)
            org_grouped_features = grouped_features
            ...
            assert self.use_xyz, "Cannot have not features and not use xyz as a feature!"
            new_features = grouped_xyz
        return grouped_xyz, new_features, org_grouped_features


class SP_Group(nn.Module):
    """
    Groups with SPQuery index
    """
    def __init__(self, use_xyz=True):
        super(SP_Group, self).__init__()
        self.use_xyz = use_xyz

    def forward(self, xyz: torch.Tensor = None, features: torch.Tensor = None, idx: torch.Tensor = None) -> torch.Tensor:
        """
        input: xyz: (b, n, 3)
               features: (b, c, n)
               idx: (b, m, k)
        output: grouped_features: (b, c, m, k)
        """
        #xyz_trans = xyz.transpose(1, 2).contiguous()    # (b, n, 3) -> (b, 3, n) 
        #grouped_xyz = grouping(xyz_trans, idx)  # (b, 3, n) with idx:(b, n, k) -> (b, 3, n, k)
        #grouped_xyz -= xyz_trans.unsqueeze(-1)  # (b, 3, n, k) - (b, 3, n, 1) -> (b, 3, n, k)
      
        grouped_features = grouping(features, idx)  # (b, c, n) with idx:(b, m, k) -> (b, c, m, k)
...
class PointNL_QueryAndGroup(nn.Module):
    """
    Groups with a ball query of radius
    parameters:
        radius: float32, Radius of ball
    ...
    """
    def __init__(self, radius=None, nsample=32, use_xyz=True):
        super(PointNL_QueryAndGroup, self).__init__()
        self.radius, self.nsample, self.use_xyz = radius, nsample, use_xyz
